...
from ._models import Product, Category, Tag, ProductImage, Specification
from .forms import ProductImageFormSet, SpecificationFormSet
...
class ProductMixin:
    ...
    def handle_formsets(self, product: Product, image_formset, spec_formset):
    ...
    def get_formsets(self, request, product=None):
        ...
        if request.method == "POST":
            image_formset = ProductImageFormSet(
                request.POST,
                request.FILES,
                queryset=ProductImage.objects.filter(product=product) if product
                else ProductImage.objects.none()
            )
            spec_formset = SpecificationFormSet(
                request.POST,
                queryset=Specification.objects.filter(product=product) if product
